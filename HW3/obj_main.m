...
function obj_main(P,p_img2,M,tex_name, mesh_name)
...
cmd_fid = ['fid = fopen(''result/' mesh_name '.obj'', ''wt'');'];
eval(cmd_fid)
fprintf(fid, '# obj file\n');
cmd_print = ['fprintf(fid, ''mtllib ' mesh_name '.mtl\n\n'');'];
eval(cmd_print);
...
% output .mtl file
%----------------------------------------------------
cmd_mtl = ['fid_mtl = fopen(''result/' mesh_name '.mtl'', ''wt'');'];
eval(cmd_mtl)
